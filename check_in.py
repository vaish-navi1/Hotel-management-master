...
def click_submit():
    ...
    if f_name == '' or l_name == '' or phone == 0 or email == '' or address == '' or room_no == 0 \
        ...
        alldb = mysql.connector.connect(host='localhost', user='root', password='123456', database='hotel')
        allcur = alldb.cursor()
        cur = mydb.cursor()
...
# Submit Button
submit_button = Button(root, text="SUBMIT", width=15, bg="#269900", fg='Black', font=('ARIAL BLACK', 10), relief=RAISED,
                       command=click_submit)
submit_button.place(relx=0.90, rely=0.95, anchor=S)
